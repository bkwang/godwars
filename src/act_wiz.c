...
void do_wizhelp( CHAR_DATA *ch, char *argument )
{
        ...
        for ( level = get_trust( ch ) ; level >= LEVEL_IMMORTAL ; level-- )
        {
                ...
                for ( cmd = 0; cmd_table[ cmd ].name[ 0 ] != '\0'; cmd++ )
                {
                        ...
                        {
                                sprintf( buf, "#C%-12s", cmd_table[ cmd ].name );
                                send_to_char( buf, ch );
                                if ( ++col % 6 == 0 )
                                        send_to_char( "\n\r", ch );
                        }
...
void do_deny( CHAR_DATA *ch, char *argument )
{
    ...
    if (release_date > 90)
    {
	send_to_char( "Deny them for more then 90 days? Why not just delete them?\n\r", ch );
	return;
    }
...
void do_info( CHAR_DATA *ch, char *argument )
{
	...
	{
	    send_to_char( "#L<#l- #RInfo #l-#L>#n ",d->character );
...
void do_pchat( CHAR_DATA *ch, char *argument )
{
	...
	if ( d->connected == CON_PLAYING && d->character != NULL )
	{
	    if (d->character == ch)
	    {
		act("To $N: You chat '#p$t#n'.",ch,arg2,victim,TO_CHAR);
		act("To others: You chat '#p$t#n'.",ch,arg3,victim,TO_CHAR);
	    }
	    else if (d->character == victim)
		act("$n chats '#p$t#n'.",ch,arg2,victim,TO_VICT);
	    else
		act("$n chats '#p$t#n'.",ch,arg3,d->character,TO_VICT);
	}
    }
    return;
}
...
void do_mset( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "  level trust exp security\n\r",			ch );
	...
	{
	    send_to_char( "Primal range is 0 to 2000.\n\r", ch );
	    return;
	}
	if (IS_JUDGE(ch) || (ch == victim)) {
...
void do_copyover (CHAR_DATA *ch, char * argument)
{
	...
	extern int port,control; /* db.c */
        ...
        if (!fp)
        {
                ...
                logx ("Could not write to copyover file");
        ...
        execl (EXE_FILE, "godwars", buf, "copyover", buf2, (char *) NULL);
...
void copyover_recover ()
{
        ...
        logx ("Copyover recovery initiated");
        ...
        if (!fp) /* there are some descriptors open which will hang forever then ? */
        {
                ...
                logx ("Copyover file not found. Exitting.\n\r");
                exit (1);
        }
...
void do_spellset( CHAR_DATA *ch, char *argument )
{
        ...
        if ( arg1[0] == '\0' || arg2[0] == '\0')
    {
        send_to_char( "#0Syntax: #Lspellset #0<person> <amount>#n\n\r", ch);
        return;
    }
    ...
    {
        stcf(ch,"Max is 15000 for spells.\n\r");
        return;
    }
...
void do_weapset( CHAR_DATA *ch, char *argument )
{
        ...
        if ( arg1[0] == '\0' || arg2[0] == '\0')
    {
        send_to_char( "#0Syntax: #Lweapset #0<person> <amount>#n\n\r",  ch);
        return;
    }
    ...
    {
        stcf(ch,"Max is 5000 for weapons.\n\r");
        return;
    }
...
void do_stanceset( CHAR_DATA *ch, char *argument )
{
        ...
        if ( arg1[0] == '\0' || arg2[0] == '\0')
    {
        send_to_char( "#0Syntax: #Lstanceset #0<person> <amount>#n\n\r",        ch);
        return;
    }
    ...
    {
        stcf(ch,"Max is 200 for stances.\n\r");
        return;
    }
