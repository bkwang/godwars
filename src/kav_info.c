...
void do_level( CHAR_DATA *ch, char *argument )
{
	...
	if (IS_SPIRIT(ch))
	    send_to_char("You are a spirit.\n\r", ch);
	if (IS_ABOMINATION(ch))
	    send_to_char("You are an abomination.\n\r", ch);
	if (IS_LICH(ch))
	    send_to_char("You are a lich.\n\r", ch);
	if (IS_SKINDANCER(ch))
	    send_to_char("You are a skindancer.\n\r", ch);
	if (IS_BAALI(ch))
	    send_to_char("You are a baali.\n\r", ch);
	if (IS_BLACK_SPIRAL_DANCER(ch))
	    send_to_char("You are a black spiral dancer.\n\r", ch);
	if (IS_NEPHANDI(ch))
	    send_to_char("You are a nephandi.\n\r", ch);
