...
void update_damcap( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	case CLASS_VAMPIRE: case CLASS_LICH:
		...
		max_dam += (get_disc(ch,DISC_POTENCE) * 100);
	    break;
	case CLASS_DEMON: case CLASS_ANGEL: case CLASS_NEPHANDI:
	    if (IS_SET(ch->act, PLR_DEMON)) max_dam += 500;
	    else if (IS_EXTRA(ch, EXTRA_PRINCE)) max_dam += 400;
	    else if (IS_EXTRA(ch, EXTRA_SIRE)) max_dam += 300;
	    else max_dam += 200;
...
/*    if ( ch->trust > LEVEL_BUILDER ) // Don't like this idea FIXME -Chrono
    {
...
//    bug( "Group_gain: members.", members );
  ...
  for ( gch = ch->in_room->people; gch != NULL; gch = gch->next_in_room )
  {
    ...
    sprintf(buf2,"#0-=-=-= #WExp modifiers  #G:");
...
//    if (!IS_SET(gch->act, PLR_BRIEF4))
    ...
    sprintf(buf2,"#0-=-=-= #WTotal modifier #R: #Y%d #Wpercent bonus#n\n\r",xp_modifier - 100);
//    if (!IS_SET(gch->act, PLR_BRIEF4))
      send_to_char(buf2, gch);
    ...
    sprintf( buf, "#R-->#0You receive #W%d #0experience points.#n\n\r", xp );
      send_to_char(buf, ch);
    gain_exp( gch, xp );
    if ((gch->exp >= 2000000000) || (gch->exp <= 0))
    {
        send_to_char("Your exp caps at 2 bill, use some of it first.\n\r", ch);
        gch->exp = 2000000000;
    }
  }
  return;
...
int xp_compute( CHAR_DATA *gch, CHAR_DATA *victim )
{
  ...
  xp = ((victim->level)*number_range(90,110))*ccenter[CCENTER_EXP_LEVEL];
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
	...
	while ( dt >= TYPE_ADD_AGG ) dt -= TYPE_ADD_AGG;
         ...
         if ( dam ==   0 ) { vs = " #cmiss#n";	vp = " #cmisses#n";		}
    else if ( dam <=  25 ) { vs = " #ylightly#n";	vp = " #ylightly#n";	}
    else if ( dam <=  50 ) { vs = " #oslightly#n";	vp = " #oslightly#n";	}
    else if ( dam <= 100 ) { vs = " #rhard#n";	vp = " #rhard#n";		}
    else if ( dam <= 250 ) { vs = " #Rvery hard#n";	vp = " #Rvery hard#n";	}
    else if ( dam <= 500 ) { vs = " #gextremely hard#n";vp = " #gextremely hard#n";}
    else                   { vs = " #Gincredibly hard#n";vp = " #gincredibly hard#n";}
	...
	(IS_NPC(victim) && IS_SET(victim->act, ACT_NOPARTS)) )
    {
	    ...
	    else if (IS_VAMPAFF(ch, VAM_CLAWS) && IS_MORE(ch, MORE_HAND_FLAME))
	    {
	    	...
	    	sprintf( buf2, "Your flaming claw hits $N%s%c #0[#R%d#0]#n", vs, punct, dam );
	    	sprintf( buf3, "$n's flaming claw hits you%s%c #0[#R%d#0]#n", vp, punct, dam );
		critical = TRUE;
	    }
	    else if (IS_VAMPAFF(ch, VAM_CLAWS))
	    {
	    	sprintf( buf1, "$n's claw hits $N%s%c",  vp, punct );
	    	sprintf( buf2, "Your claw hits $N%s%c #0[#R%d#0]#n", vs, punct, dam );
	    	sprintf( buf3, "$n's claw hits you%s%c #0[#R%d#0]#n", vp, punct, dam );
		critical = TRUE;
	    }
	    else if (IS_MORE(ch, MORE_HAND_FLAME))
	    {
	    	sprintf( buf1, "$n's flaming hand hits $N%s%c",  vp, punct );
	    	sprintf( buf2, "Your flaming hand hits $N%s%c #0[#R%d#0]#n", vs, punct, dam );
	    	sprintf( buf3, "$n's flaming hand hits you%s%c #0[#R%d#0]#n", vp, punct, dam );
		critical = TRUE;
	    }
	    else
	    {
	    	sprintf( buf1, "$n hits $N%s%c",  vp, punct );
	    	sprintf( buf2, "You hit $N%s%c #0[#R%d#0]#n", vs, punct, dam );
	    	sprintf( buf3, "$n hits you%s%c #0[#R%d#0]#n", vp, punct, dam );
		critical = TRUE;
	    }
    	}
    	else
		...
		if ( dt >= 0 && dt < MAX_SKILL )
		{
	    	    ...
	    	    sprintf( buf2, "Your %s strikes $N%s%c #0[#R%d#0]#n",  attack, vp, punct, dam );
	    	    sprintf( buf3, "$n's %s strikes you%s%c #0[#R%d#0]#n", attack2, vp, punct, dam );
		}
		else
		{
	    	    sprintf( buf1, "$n %s $N%s%c",  attack2, vp, punct );
	    	    sprintf( buf2, "You %s $N%s%c #0[#R%d#0]#n",  attack, vp, punct, dam );
	    	    sprintf( buf3, "$n %s you%s%c #0[#R%d#0]#n", attack2, vp, punct, dam );
		    critical = TRUE;
		}
	    }
...
void do_decapitate( CHAR_DATA *ch, char *argument )
{
	...
	ch->paradox[2] += PARADOX_TICK;
    	sprintf(buf,"#r%s#n has been decapitated by #r%s#n for no status.",victim->name,ch->name);
    	do_fatality(ch,buf);
...
void death_message( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    {
	default: 
	    sprintf(buf,"#r%s#n has just died!",chname);
	    break;
	case 1:
	    sprintf(buf,"#r%s#n has just been dissolved by #r%s#n's acid blood!",victim->name,chname);
	    break;
	case 2:
	    sprintf(buf,"#r%s#n just committed suicide by tearing %s own heart out!",chname,chsex);
	    break;
	case 3:
	    sprintf(buf,"#r%s#n has just sucked out all of #r%s#n's blood!",chname,victim->name);
	    break;
	case 4:
	    sprintf(buf,"#r%s#n has just boiled all of #r%s#n's blood!",chname,victim->name);
	    break;
	case 5:
	    sprintf(buf,"#r%s#n has just turned #r%s#n's blood into water!",chname,victim->name);
	    break;
	case 6:
	    sprintf(buf,"#r%s#n has just evaporated all of #r%s#n's blood!",chname,victim->name);
	    break;
	case 7:
	    sprintf(buf,"#r%s#n has just smashed #r%s#n to death!",chname,victim->name);
	    break;
	case 8:
	    sprintf(buf,"#r%s#n has just blown apart #r%s#n's mind!",chname,victim->name);
	    break;
	case 9:
	    sprintf(buf,"#r%s#n has just torn out #r%s#n's soul!",chname,victim->name);
	    break;
	case 10:
	    sprintf(buf,"#r%s#n has just sucked out #r%s#n's life essence!",chname,victim->name);
	    break;
	case 11:
	    sprintf(buf,"#r%s#n has just blown apart #r%s#n's eardrums!",chname,victim->name);
	    break;
	case 12:
	    sprintf(buf,"#r%s#n's entrails just poured from %s torn open stomach!",chname,chsex);
	    break;
	case 13:
	    sprintf(buf,"#r%s#n just died from %s wounds!",chname,chsex);
	    break;
	case 14:
	    sprintf(buf,"#r%s#n just burned to death!",chname);
	    break;
	case 16:
	    sprintf(buf,"#r%s#n just died a long, slow, painful death!",chname);
	    break;
	case 17:
	    sprintf(buf,"#r%s#n has just torn apart #r%s#n's body!",chname,victim->name);
	    break;
	case 18:
	    sprintf(buf,"#r%s#n has slain #r%s#n in cold blood!",chname,victim->name);
	    break;
	case 19:
	    sprintf(buf,"#r%s#n has just shot down #r%s#n!",chname,victim->name);
	    break;
	case 20:
	    sprintf(buf,"#r%s#n has struck #r%s#n a mortal wound!",chname,victim->name);
	    break;
	case 21:
	    sprintf(buf,"#r%s#n has torn off #r%s#n's face!",chname,victim->name);
	    break;
	case 22:
	    sprintf(buf,"#r%s#n has ripped out #r%s#n's windpipe!",chname,victim->name);
	    break;
	case 23:
	    sprintf(buf,"#r%s#n has ripped out #r%s#n's entrails!",chname,victim->name);
	    break;
	case 24:
	    sprintf(buf,"#r%s#n has broken #r%s#n's back!",chname,victim->name);
	    break;
	case 25:
	    sprintf(buf,"#r%s#n has broken #r%s#n's neck!",chname,victim->name);
	    break;
	case 26:
	    sprintf(buf,"#r%s#n has disembowelled #r%s#n!",chname,victim->name);
	    break;
	case 27:
	    sprintf(buf,"#r%s#n has thrust %s weapon through #r%s#n's head!",chname,chsex,victim->name);
	    break;
	case 28:
	    sprintf(buf,"#r%s#n has sliced open #r%s#n's carotid artery!",chname,victim->name);
	    break;
	case 29:
	    sprintf(buf,"#r%s#n has sliced open #r%s#n's throat!",chname,victim->name);
	    break;
	case 30:
	    sprintf(buf,"#r%s#n has split open #r%s#n's head!",chname,victim->name);
	    break;
	case 31:
	    sprintf(buf,"#r%s#n has thrust %s weapon through #r%s#n's chest!",chname,chsex,victim->name);
	    break;
	case 32:
	    sprintf(buf,"#r%s#n has swung %s weapon between #r%s#n's legs!",chname,chsex,victim->name);
	    break;
	case 33:
	    sprintf(buf,"#r%s#n has sliced off #r%s#n's arm!",chname,victim->name);
	    break;
	case 34:
	    sprintf(buf,"#r%s#n has sliced off #r%s#n's leg!",chname,victim->name);
	    break;
	case 35:
	    sprintf(buf,"#r%s#n has thrust %s weapon through #r%s#n's mouth!",chname,chsex,victim->name);
	    break;
	case 36:
	    sprintf(buf,"#r%s#n has thrust %s weapon through #r%s#n's eye!",chname,chsex,victim->name);
	    break;
	case 37:
	    sprintf(buf,"#r%s#n has smashed in #r%s#n's chest!",chname,victim->name);
	    break;
	case 38:
	    sprintf(buf,"#r%s#n has shattered #r%s#n's spine!",chname,victim->name);
	    break;
	case 39:
	    sprintf(buf,"#r%s#n has torn out #r%s#n's throat with %s teeth!",chname,victim->name,chsex);
	    break;
	case 40:
	    sprintf(buf,"#r%s#n has torn out #r%s#n's throat with %s claws!",chname,victim->name,chsex);
	    break;
	case 41:
	    sprintf(buf,"#r%s#n has clawed out #r%s#n's eyeball!",chname,victim->name);
	    break;
	case 42:
	    sprintf(buf,"#r%s#n has torn open #r%s#n's chest with %s claws!",chname,victim->name,chsex);
	    break;
	case 43:
	    sprintf(buf,"#r%s#n has sucked out #r%s#n's brains!",chname,victim->name);
	    break;
    }
    buf[0] = UPPER( buf[0] );
    do_fatality(ch,buf);
    ...
    {
	sprintf(buf,"#r%s#n's unborn child has been slain!",victim->name);
	buf[0] = UPPER( buf[0] );
	do_fatality(ch,buf);
    }
    return;
