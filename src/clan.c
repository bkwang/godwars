...
void do_pray( CHAR_DATA *ch, char *argument )
{
	...
	if ( d->connected != CON_PLAYING ) continue;
	if ( (victim = d->character) == NULL ) continue;
	if ( IS_NPC(victim) || (!IS_IMMORTAL(victim) && !IS_DEMON(victim) ))
continue;
	if ( IS_SET(victim->deaf, CHANNEL_PRAY) ) continue;
	if ( victim == ch )
	    act("You pray '#c$T#n'.", ch, NULL, argument, TO_CHAR);
	else
	{
	    sprintf(buf,"$n prays '#c%s#n'.", argument);
...
bool birth_ok( CHAR_DATA *ch, char *argument )
{
	...
	sprintf(buf,"#r%s#n has given birth to #r%s#n's son, named #r%s#n!",ch->name,dad,child);
	do_info(ch,buf);
	return TRUE; 
    }
    else if (ch->pcdata->genes[4] == SEX_FEMALE)
	...
	sprintf(buf,"#r%s#n has given birth to #r%s#n's daughter, named #r%s#n!",ch->name,dad,child);
	do_info(ch,buf);
	return TRUE; 
    }
    return FALSE;
