	...
	if ( !IS_NPC(ch) )
	{
		...
		else
		{
			...
			if (!IS_NPC(vch) && IS_VAMPIRE(vch))
			{
			    ...
			    sprintf( buf, "#r%s#n has been diablerised by #r%s#n.", vch->name, ch->name);
			    do_info(ch, buf);
			    if (ch->beast == 0)
			    {
				send_to_char("You have lost Golconda!\n\r", ch);
				sprintf(buf, "#r%s#n has lost Golconda!", ch->name);
				do_info(ch,buf);
...
void update_handler( void )
{
	...
	for ( d = descriptor_list; d != NULL; d = d->next )
	{
	    ...
	    &&  !IS_NPC(ch))
	    {
//		ch->exp += ch->beast ? 1 : 10; /* FIXME -Chrono Gives exp for having beast? WTF */
...
void paradox( CHAR_DATA *ch )
{
    ...
    sprintf( buf, "#r%s#n is struck by a paradox.\n\r", ch->name);
    do_info(ch, buf);
    ch->paradox[1] = 0;
    ch->paradox[2] = 0;
