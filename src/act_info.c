...
char *format_obj_to_char( OBJ_DATA *obj, CHAR_DATA *ch, bool fShort )
{
    ...
    else if ( IS_SET(obj->quest, QUEST_RELIC)) strcat(buf, "#r(Relic)#n " );
    else if ( obj->points > 0 )
    {
	if (IS_SET(obj->quest, QUEST_IMPROVED)) strcat(buf, "#P(Mythical)#n " );
	else strcat(buf, "#p(Legendary)#n " );
    }
    if ( IS_SET(obj->quest, QUEST_BLOODAGONY)) strcat(buf, "#r(Bleeding)#n " );
    if ( IS_OBJ_STAT(obj, ITEM_INVIS)     )   strcat( buf, "#c(Invis)#n " );
    if ( IS_AFFECTED(ch, AFF_DETECT_EVIL)
         && !IS_OBJ_STAT(obj, ITEM_ANTI_GOOD)
         && IS_OBJ_STAT(obj, ITEM_ANTI_EVIL)   )   strcat( buf, "#l(Blue Aura)#n "  );
    ...
    if ( IS_AFFECTED(ch, AFF_DETECT_MAGIC)
         && IS_OBJ_STAT(obj, ITEM_MAGIC)  )   strcat( buf, "#G(Magical)#n " );
    if ( IS_OBJ_STAT(obj, ITEM_GLOW)      )   strcat( buf, "#g(Glowing)#n " );
    if ( IS_OBJ_STAT(obj, ITEM_HUM)       )   strcat( buf, "#g(Humming)#n " );
    if ( IS_OBJ_STAT(obj, ITEM_SHADOWPLANE) &&
	 obj->in_room != NULL &&
	!IS_AFFECTED(ch,AFF_SHADOWPLANE) )    strcat( buf, "#C(Umbra)#n " );
    if (!IS_OBJ_STAT(obj, ITEM_SHADOWPLANE) &&
	 obj->in_room != NULL &&
	 IS_AFFECTED(ch,AFF_SHADOWPLANE) )    strcat( buf, "#C(Normal plane)#n " );
    if ( !IS_NPC(ch) && IS_VAMPIRE(ch) && 
	IS_OBJ_STAT(obj, ITEM_VANISH) &&
	get_disc(ch,DISC_CHIMERSTRY) > 0 )    strcat( buf, "#C(Illusion)#n " );
    if ( !IS_NPC(ch) && IS_WEREWOLF(ch) && 
	IS_SET(obj->spectype, SITEM_SILVER) &&
	get_auspice(ch,AUSPICE_AHROUN) > 1 )  strcat( buf, "#7(Silver)#n " );
...
int get_disc( CHAR_DATA *ch, int disc )
{
	...
	{
	    default:		 max = 4; break;
	    case DISC_AUSPEX:	 max = 5; break;
	    case DISC_DAIMOINON: max = (!IS_ANGEL(ch)) ? 5 : 4; break;
	    case DISC_OBEAH:	 max = (IS_ANGEL(ch))  ? 5 : 4; break;
	}
    }
    else if (ch->class != CLASS_VAMPIRE) max = 4;
...
int char_damroll( CHAR_DATA *ch )
{
	...
	else if (IS_POLYAFF(ch, POLY_SERPENT)) dr += 150;
...
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch )
{
	...
	if ( !IS_NPC(ch) && IS_WEREWOLF(ch) && 
	   (IS_ITEMAFF(victim, ITEMA_LEFT_SILVER) ||
	    ...
	    get_auspice(ch,AUSPICE_AHROUN) > 1    ) strcat( buf, "(Silver) "   );
    }
	...
	{
	    default:				break;
	    case CLASS_VAMPIRE:			strcat( buf, "(Vampire) " ); break;
	    case CLASS_WEREWOLF:		strcat( buf, "(Werewolf) " ); break;
	    case CLASS_ABOMINATION:		strcat( buf, "(Abomination) " ); break;
	    case CLASS_MAGE:			strcat( buf, "(Mage) " ); break;
	    case CLASS_LICH:			strcat( buf, "(Lich) " ); break;
	    case CLASS_SKINDANCER:		strcat( buf, "(Skindancer) " ); break;
	    case CLASS_DEMON:			strcat( buf, "(Demon) " ); break;
	    case CLASS_BAALI:			strcat( buf, "(Baali) " ); break;
	    case CLASS_BLACK_SPIRAL_DANCER:	strcat( buf, "(Black Spiral Dancer) " ); break;
	    case CLASS_NEPHANDI:		strcat( buf, "(Nephandi) " ); break;
	    case CLASS_HIGHLANDER:		strcat( buf, "(Highlander) " ); break;
	    case CLASS_GHOUL:			strcat( buf, "(Ghoul) " ); break;
	    case CLASS_ANGEL:			strcat( buf, "(Angel) " ); break;
	}
    }
...
void show_char_to_char_1( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    else if (IS_ANGEL(victim) && IS_VAMPAFF(victim, VAM_NIGHTSIGHT)) act("$N's eyes are glowing bright blue.",ch,NULL,victim,TO_CHAR);
...
void do_exits( CHAR_DATA *ch, char *argument )
{
	...
	&&   !IS_SET(pexit->exit_info, EX_CLOSED) )
	{
		...
		else sprintf( buf + strlen(buf), "#y%-5s #r-#n %s\n\r",
		    capitalize( dir_name[door] ),
...
void do_score( CHAR_DATA *ch, char *argument )
{
    ...
    {
        send_to_char( "You are ", ch );
        if      (ch->level == 1 ) send_to_char( "a Mortal.\n\r", ch);
        else if (ch->level == 2 ) send_to_char( "a Mortal.\n\r", ch);
        else if (ch->level == 7 ) send_to_char( "a Builder.\n\r", ch);
        else if (ch->level == 8 ) send_to_char( "a Quest Maker.\n\r", ch);
        else if (ch->level == 9 ) send_to_char( "an Enforcer.\n\r", ch);
        else if (ch->level == 10) send_to_char( "a Judge.\n\r", ch);
        else if (ch->level == 11) send_to_char( "a High Judge.\n\r", ch);
        else if (ch->level == 12) send_to_char( "an Implementor.\n\r", ch);
        else if (ch->race <= 0 ) send_to_char( "an Avatar.\n\r", ch);
        else if (ch->race <= 4 ) send_to_char( "an Immortal.\n\r", ch);
        else if (ch->race <= 9 ) send_to_char( "a Godling.\n\r", ch);
        else if (ch->race <= 14) send_to_char( "a Demigod.\n\r", ch);
        else if (ch->race <= 19) send_to_char( "a Lesser God.\n\r", ch);
        else if (ch->race <= 24) send_to_char( "a Greater God.\n\r", ch);
        else if (ch->race >= 25) send_to_char( "a Supreme God.\n\r", ch);
    ...
    if ( !IS_NPC(ch) )
    {
        ...
        else if ( IS_DEMON(ch) && ch->pcdata->demonic > 0 )
        {
            ...
            if (ch->class == CLASS_DEMON || ch->class == CLASS_ANGEL) bonus <<=1;
...
void do_time( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf,
	"It is %d o'clock %s, Day of %s, %d%s the Month of %s.\n\rGodWars 2000 started up at %s\rThe system time is %s\r",
...
void do_who( CHAR_DATA *ch, char *argument )
{
	...
	while (arg2[loop++] != '\0'){if (arg2[loop] == ' ') arg2[loop] = '*';}
    }
	...
	if ( is_number( arg ) )
	{
	    send_to_char("Type 'who mortal' for mortals, or 'who immortal' for Immortals.\n\r",ch);
	    return;
	}
	else
	{
	    ...
	    wch->pcdata->chobj != NULL )
	{
	    ...
	    {
		default:	class = "#gAn Object#n    ";	break;
		case 12:	class = "#gA Head#n       ";	break;
		case 30005:	class = "#gA Brain#n      ";	break;
	    }
	}
	...
	case MAX_LEVEL - 10:
	case MAX_LEVEL - 11:
	case MAX_LEVEL - 12: class = "#gMortal#n       "; break;
	}
	/*
	 * Format it up.
	 */
...
*/
	{
		...
		else sprintf( kav, ". %sRonin%s", openb, closeb );
	    }
	    else if ( IS_MAGE(wch) )
	    {
		char *mage_col;
		...
		{
		    default:
		    case PURPLE_MAGIC:	mage_col = "Air";	break;
		    case RED_MAGIC:	mage_col = "Fire";	break;
		    case BLUE_MAGIC:	mage_col = "Water";	break;
		    case GREEN_MAGIC:	mage_col = "Earth";	break;
		    case YELLOW_MAGIC:	mage_col = "Lightning";	break;
		}
		switch ( wch->level )
		{
		    default:
...
*/
    ...
    {
	if ((nTotal - nMatch) < 2)
	    sprintf( buf2, "#cThere is one invisible player connected.#m\n\r");
	else
	    sprintf( buf2, "#cThere are a total of %d invisible players connected.#n\n\r", (nTotal - nMatch));
	send_to_char( buf2, ch );
    }
    send_to_char( "#l--------------------------------------------------------------------------------#n\n\r", ch );
    return;
...
void do_practice( CHAR_DATA *ch, char *argument )
{
	...
	/* Added so you can practice a skill to 100% in one pop - Kai */
...
void do_channels( CHAR_DATA *ch, char *argument )
{
	...
	if (IS_WEREWOLF(ch) || IS_IMMORTAL(ch))
	{
	...
	else
	{
	    ...
	    return;
	     ...
	     if ( !str_cmp( arg+1, "magetalk" ) ) bit = CHANNEL_MAGETALK;
	    ...
	    !str_cmp( arg+1, "pray") ) bit = CHANNEL_PRAY;
...
void do_config( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char(  IS_SET(ch->act, PLR_ANSI)
            ? "#L[#G+ANSI     #L] #0You have ansi colour on.\n\r"
	    : "#L[#r-ansi     #L] #0You have ansi colour off.\n\r"
	    , ch );

	send_to_char(  IS_SET(ch->act, PLR_AUTOEXIT)
            ? "#L[#G+AUTOEXIT #L] #0You automatically see exits.\n\r"
	    : "#L[#r-autoexit #L] #0You don't automatically see exits.\n\r"
	    , ch );

	send_to_char(  IS_SET(ch->act, PLR_AUTOLOOT)
	    ? "#L[#G+AUTOLOOT #L] #0You automatically loot corpses.\n\r"
	    : "#L[#r-autoloot #L] #0You don't automatically loot corpses.\n\r"
	    , ch );

	send_to_char(  IS_SET(ch->act, PLR_AUTOSAC)
	    ? "#L[#G+AUTOSAC  #L] #0You automatically sacrifice corpses.\n\r"
	    : "#L[#r-autosac  #L] #0You don't automatically sacrifice corpses.\n\r"
	    , ch );

	send_to_char(  IS_SET(ch->act, PLR_BLANK)
	    ? "#L[#G+BLANK    #L] #0You have a blank line before your prompt.\n\r"
	    : "#L[#r-blank    #L] #0You have no blank line before your prompt.\n\r"
	    , ch );

	send_to_char(  IS_SET(ch->act, PLR_BRIEF)
	    ? "#L[#G+BRIEF    #L] #0You see brief descriptions.\n\r"
	    : "#L[#r-brief    #L] #0You see long descriptions.\n\r"
	    , ch );
         
	send_to_char(  IS_SET(ch->act, PLR_COMBINE)
	    ? "#L[#G+COMBINE  #L] #0You see object lists in combined format.\n\r"
	    : "#L[#r-combine  #L] #0You see object lists in single format.\n\r"
	    , ch );

	send_to_char(  IS_SET(ch->act, PLR_PROMPT)
	    ? "#L[#G+PROMPT   #L] #0You have a prompt.\n\r"
	    : "#L[#r-prompt   #L] #0You don't have a prompt.\n\r"
	    , ch );
        send_to_char(IS_SET(ch->act, PLR_MAP)
	    ? "#L[#G+MAP      #L] #0You see the automapper.\n\r"
	    : "#L[#r-map      #L] #0You don't see the automapper.\n\r", ch);

	send_to_char(  IS_SET(ch->act, PLR_TELNET_GA)
	    ? "#L[#G+TELNETGA #L] #0You receive a telnet GA sequence.\n\r"
	    : "#L[#r-telnetga #L] #0You don't receive a telnet GA sequence.\n\r"
	    , ch );

	send_to_char(  IS_SET(ch->act, PLR_SILENCE)
	    ? "#L[#R+SILENCE  #L] #0You are silenced.\n\r"
	    : ""
	    , ch );

	send_to_char( !IS_SET(ch->act, PLR_NO_EMOTE)
	    ? ""
	    : "#L[#R-emote    #L] #0You can't emote.\n\r"
	    , ch );

	send_to_char( !IS_SET(ch->act, PLR_NO_TELL)
	    ? ""
	    : "#L[#R-tell     #L] #0You can't use 'tell'.\n\r"
	    , ch );
    }
    else
    {
...
void do_affects( CHAR_DATA *ch, char *argument )
{
    ...
    else send_to_char( "You don't have any special affects on you.\n\r", ch );
    return;
}
