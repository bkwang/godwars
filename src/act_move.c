...
void do_escape( CHAR_DATA *ch, char *argument )
{
    ...
    do_look( ch, "auto" );
    sprintf(buf,"#r%s#n has escaped defenceless from a fight.",ch->name);
    do_info(ch,buf);
...
/*    if ( arg2[0] != '\0' && is_ok )
  ...
  else if (!str_cmp(arg1,"hp"))
  {
    ...
    int urin = 0;
    int urin_counter = 0;
    int statcap = UMIN(50000, 50000 + 1000 * ch->race);
  ...
  else if (!str_cmp(arg1,"mana"))
  {
	...
	if ( ch->level == 2 )
	{
        ...
        {
           sprintf( buf, "#WHp               #R- #Y%d #0exp per point.#n\n\r",(ch->max_hit+1));
           send_to_char( buf, ch );
        }

        if ( ch->max_mana      < (300000) )
        {
           sprintf( buf, "#WMana             #R- #Y%d #0exp per point.#n\n\r",(ch->max_mana+1));
           send_to_char( buf, ch );
        }
        if ( ch->max_move      < (300000) )
        {
           sprintf( buf, "#WMove             #R- #Y%d #0exp per point.#n\n\r",(ch->max_move+1));
           send_to_char( buf, ch );
        }
        if ( ch->practice        < 4000 )
        {
           sprintf( buf, "#WPrimal           #R- #Y%d #0exp per point of primal energy.#n\n\r", (1+ch->practice)*500 );
           send_to_char( buf, ch );
        }
    ...
    *pAbility		+= 1;
	...
	sprintf( buf, "#r%s#n has become an avatar!\n\r",ch->name );
	do_info(ch,buf);
    }
    else if (last)
...
void do_tie( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf(buf,"#r%s#n has been tied up by #r%s#n.",victim->name,ch->name);
    do_info(ch,buf);
    return;
}
...
void add_tracks( CHAR_DATA *ch, int direction )
{
    ...
    if (IS_NPC(ch)) return;
    if (IS_WEREWOLF(ch) && get_auspice(ch, AUSPICE_RAGABASH) > 0) return;
