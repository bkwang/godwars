...
/*    Name,           Level,  Trust,  Security,  Message                   */
   {  "Chrono",    MAX_LEVEL,    MAX_LEVEL,         9,  "Welcome Back, Chrono.#n\n\r"  },
   {  "Ruinz",    MAX_LEVEL,    MAX_LEVEL,         9,  "Welcome Back, Dark Lord Ruinz.#n\n\r"  },
   /* Leave this in */
   { "", 0, 0, 0, "" }
};

void do_showrelevel( CHAR_DATA *ch, char *argument )
{
  char buf[MSL];
  int member = 0;

        send_to_char("#0-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-#n\n\r",ch);
        send_to_char("#0-= #CName               #RLevel #YTrust #cSecurity      #WMessage   #0-=-#n\n\r",ch);
        send_to_char("#0-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-#n\n\r",ch);
        send_to_char("\n\r",ch);

   for( member = 0; relevel_details[member].name[0] != '\0'; member++ )
        {
         sprintf(buf,"#C%-9s                    #R%d    #Y%d    #c%d    #W%s#n", relevel_details[member].name,
relevel_details[member].level, relevel_details[member].trust, relevel_details[member].security,
relevel_details[member].message);
         send_to_char(buf,ch);
        }
        return;
}

void do_relevel( CHAR_DATA *dMob, char * arg )
{
   bool found = FALSE;
   int member = 0;
...
void do_accept( CHAR_DATA *ch, char *argument )
{
	...
	do_autosave(ch,"");
	do_autosave(victim,"");
	sprintf(buf,"#r%s#n and #r%s#n are now engaged!",ch->name,victim->name);
	do_info(ch,buf);
	return;
    }
    send_to_char("I don't think that would be a very good idea...\n\r",ch);
...
void do_breakup( CHAR_DATA *ch, char *argument )
{
	...
	do_autosave(ch,"");
	do_autosave(victim,"");
	sprintf(buf,"#r%s#n and #r%s#n have broken up!",ch->name,victim->name);
	do_info(ch,buf);
	return;
    }
...
void do_marry( CHAR_DATA *ch, char *argument )
{
	...
	do_autosave(victim1,"");
	do_autosave(victim2,"");
	sprintf(buf,"#r%s#n and #r%s#n are now married!",victim1->name,victim2->name);
	do_info(ch,buf);
	return;
    }
...
void do_divorce( CHAR_DATA *ch, char *argument )
{
	...
	do_autosave(victim1,"");
	do_autosave(victim2,"");
	sprintf(buf,"#r%s#n and #r%s#n are now divorced!",victim1->name,victim2->name);
	do_info(ch,buf);
	return;
    }
    send_to_char("But they are not married!\n\r",ch);
...
void do_quest( CHAR_DATA *ch, char *argument )
{
	...
	if ( arg3[0] == '\0' )
	{
        ...
        if ( value < 0 )
        {
	    send_to_char("You cannot use negative numbers here.\n\r",ch);
	    return;
	}
	if (obj->item_type != ITEM_WEAPON)
	{
...
void do_ccenter(CHAR_DATA * ch, char *argument)
{
  ...
  if (arg1[0] == '\0')
  {
    sprintf(buf, "  #0[#G***#0]   #YGodWars Control Center   #0[#G***#0]#n\n\r\n\r");
    send_to_char(buf, ch);
  ...
  if (!str_cmp(arg1, "lexp"))
 {
  ...
  {
    ccenter[CCENTER_MIN_EXP] = 50000;
    ccenter[CCENTER_MAX_EXP] = 300000;
    ccenter[CCENTER_EXP_LEVEL] = 1100;
