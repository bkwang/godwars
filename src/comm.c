...
const  char  do_term_type_str[] = { IAC, DO, TELOPT_TTYPE, '\0'};
const  char  term_type_sub_send[] = {IAC, SB, TELOPT_TTYPE, TELQUAL_SEND, IAC, SE, '\0'};
...
# define min(a,b)     (((a) < (b)) ? (a) : (b))
...
#if defined(macintosh) || defined(MSDOS)
    boot_db( );
    log_string( "GodWars 2000 is ready to rock." );
    game_loop_mac_msdos( );
#endif
    ...
    boot_db( );
    sprintf( log_buf, "GodWars 2000 is ready to rock on port %d.", port );
    log_string( log_buf );
    if (fCopyOver)
        copyover_recover();
...
#define FNDELAY O_NDELAY
    ...
    *dnew		= d_zero;
    ...
    dnew->character	= NULL;
//    dnew->connected     = CON_AUTO_ANSI;
    dnew->connected	= CON_GET_NAME;
//    dnew->autoansi      = PULSE_PER_SECOND * 2.5;
    dnew->outsize	= 2000;
    dnew->pEdit		= NULL;	/* OLC */
  ...
  //  write_to_buffer(dnew, do_term_type_str, 0);
	...
	for(i = 0; d->inbuf[i] != '\0'; i++)
	{
		if(d->inbuf[i] == (char)IAC)
		{
			...
			case (char)WILL:
				...
				{
					write_to_buffer(d, term_type_sub_send, 0);
				}
				i++;
				break;
			case (char)WONT:
				break;
      	        			case (char)SB:				/* Check for sub-negotiation */
				i++;
				if(d->inbuf[i] == TELOPT_TTYPE)
				{
					i += 2;				/* skip IS */
					j = 0;
					while(d->inbuf[i] != (char)IAC)  
					{
						/* Put the terminal type in the players command string */
						d->incomm[j] = tolower(d->inbuf[i]);
						j++;
						i++;
					}
					d->incomm[j] = '\0';
...
bool process_output( DESCRIPTOR_DATA *d, bool fPrompt )
{
	...
	else if ( IS_SET(ch->act, PLR_PROMPT) )
	{
		...
		sprintf(exp_str, "%d", ch->exp);
		COL_SCALE(exp_str, ch, ch->exp, 100);
	        sprintf( buf, "<[%sX] [%sH %sM %sV]> \n\r",exp_str, hit_str, mana_str, move_str );
	    }
...
*/
	if ( ch->level == 0 )
	{
	    ...
	    do_look( ch, "auto" );
	    /* Newbies get black shimmering armor - Kai */
	    send_to_char("\n\r", ch);
	    obj_to_char(create_object(get_obj_index( 30333 ), 1), ch);
	    obj_to_char(create_object(get_obj_index( 30334 ), 1), ch);
	    obj_to_char(create_object(get_obj_index( 30335 ), 1), ch);
	    obj_to_char(create_object(get_obj_index( 30336 ), 1), ch);
	    obj_to_char(create_object(get_obj_index( 30337 ), 1), ch);
	    obj_to_char(create_object(get_obj_index( 30338 ), 1), ch);
	    obj_to_char(create_object(get_obj_index( 30339 ), 1), ch);
	    obj_to_char(create_object(get_obj_index( 30339 ), 1), ch);
	    obj_to_char(create_object(get_obj_index( 30340 ), 1), ch);
	    obj_to_char(create_object(get_obj_index( 30340 ), 1), ch);
	    obj_to_char(create_object(get_obj_index( 30342 ), 1), ch);
	    obj_to_char(create_object(get_obj_index( 30342 ), 1), ch);
	    obj_to_char(create_object(get_obj_index( 30343 ), 1), ch);
	    obj_to_char(create_object(get_obj_index( 30343 ), 1), ch);
	    do_wear( ch, "all" );
	    send_to_char("\n\r", ch);
	}
	...
	sprintf(buf,"#R%s#n has entered GodWars 2006.", ch->name);
	do_info(ch,buf);
	act( "$n has entered the game.", ch, NULL, NULL, TO_ROOM );
        i = num_changes();
        if(i > 0)
	{
	   sprintf(buf, "#R+++ #7There were #R[#f%d#R] #n#7changes made to the mud today type #GCHANGES#7 to view #R+++#n\n\r", i);
	   stc(buf, ch);
	}
...
bool check_parse_name( char *name )
{
    ...
    if (is_in(name,"|shit*fuck*cunt*assh*bitch*suck*dick*"))
	return FALSE;
 ...
 * Write to one char.
 */
void stcf(CHAR_DATA *ch,const char *fmt,...)
{
    char buf[MAX_STRING_LENGTH];
    va_list args;
