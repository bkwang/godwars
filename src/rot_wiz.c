...
void do_pset( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "  quest weapon immune beast dps status\n\r",     ch );
	...
	send_to_char( "Immune:  hurl backstab shielded kick disarm\n\r",  ch );
        ...
        send_to_char( "         Red Blue Green Yellow Abomination\n\r",   ch );
        send_to_char( "         Skindancer Lich Baali Bsd Nephandi\n\r",  ch );
        send_to_char( "         Highlander Ghoul\n\r",			  ch );
	return;
    }
    ...
    if ( !str_cmp( arg2, "status" ) )
    {
        ...
        if ( value < 0 || value > 25 )
        {
            send_to_char( "Status range is 0 to 25.\n\r", ch );
            return;
        }
